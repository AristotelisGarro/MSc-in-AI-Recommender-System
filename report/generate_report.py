...
<style>
  ...
  ol.references li {{ margin: 10px 0; font-size: 10pt; line-height: 1.5; color: #333; }}
</style>
</head>
<body>
...
<div class="cover">
  ...
  <p class="meta"><strong>Φοιτητής:</strong> Γκάρο Αριστοτέλης &nbsp;|&nbsp; <strong>ΑΜ:</strong> mtn2503</p>
...
<ul>
  ...
  <li><strong>Περιορισμένο cold-start σενάριο:</strong> Το dataset ορίζει minimum 20 ratings ανά
  χρήστη — κανένας χρήστης δεν έχει μηδενικό ιστορικό. Για το Switching Hybrid ορίζουμε
  threshold=50 ως όριο cold-start, με αποτέλεσμα 36.1% των χρηστών (&lt;50 ratings στο train)
  να δρομολογούνται στο CB και 63.9% στο CF.</li>
...
<ul>
  ...
  <li>Σε dataset με πραγματικούς cold-start χρήστες (0-5 ratings), το βέλτιστο α θα ήταν
...
Grid search σε thresholds [5, 10, 15, 20, 30, 50]. Επειδή το dataset εγγυάται minimum
20 ratings ανά χρήστη, thresholds ≤ 16 (≈80% του 20) δεν ενεργοποιούνται ποτέ —
ισοδυναμούν με pure CF. Επιλέγουμε <strong>threshold = 50</strong> ως ουσιαστικό όριο
cold-start που δημιουργεί αξιόλογο switching.
</div>
<p>
Η λογική του Switching Hybrid είναι η <strong>adaptive εναλλαγή</strong>: για χρήστες
με λίγες αξιολογήσεις (cold-start) χρησιμοποιούμε CB (αξιόπιστο ακόμα και με λίγα ratings),
ενώ για χρήστες με πλούσιο ιστορικό εφαρμόζουμε CF (πιο ακριβές). Με threshold=50,
36.1% των χρηστών (2.182 από 6.040) δρομολογούνται στο CB και 63.9% στο CF.
</p>
<div class="finding">
<strong>Switching Hybrid (threshold=50) — Αποτέλεσμα:</strong>
RMSE=0.8897, ελαφρώς χειρότερο από pure CF (0.8784). Αυτό επιβεβαιώνει ότι
ακόμα και για χρήστες με 20–49 ratings στο train, το SVD παράγει καλύτερες
προβλέψεις από το genre-based CB. Το Switching Hybrid θα αποδείκνυε την αξία
του σε dataset με πραγματικούς νέους χρήστες (0–5 ratings), όπου το CF αδυνατεί.
</div>
...
{row(f"Weighted Hybrid (α={alpha:.2f})",
     ...
     bold=True)}
<tr><td>Switching Hybrid (thr=50)</td><td>0.8897</td><td>0.6980</td>
    <td>0.7737</td><td>0.4224</td><td>0.5465</td>
    <td>0.6835</td><td>0.6347</td><td>0.6582</td></tr>
</tbody></table>
...
<ul>
  ...
  <li>Το <strong>Switching Hybrid (thr=50) επιτυγχάνει RMSE=0.8897</strong>: 36.1% χρηστών
  (&lt;50 ratings στο train) δρομολογούνται στο CB, 63.9% στο CF. Ελαφρώς χειρότερο από pure
  CF — αποδεικνύει ότι ακόμα και για σχετικά sparse χρήστες, το SVD υπερέχει του genre-based CB.</li>
...
<ul>
  ...
  (genre-only) προσφέρει σταθερή βελτίωση. Το Switching Hybrid (thr=50, RMSE=0.8897)
  είναι ελαφρώς χειρότερο από pure CF — επιβεβαιώνει ότι το SVD κυριαρχεί ακόμα και
  για χρήστες με 20–49 ratings, ενώ το CB θα άξιζε σε αληθινά cold-start σενάρια.</li>
</ul>
...
<ul>
  ...
  με &lt;20 ratings — δεν αξιολογείται το ακραίο cold-start σενάριο (0–5 ratings) όπου
  το Switching Hybrid θα είχε μεγαλύτερο πλεονέκτημα έναντι του pure CF.</li>
...
<!-- 7. ΒΙΒΛΙΟΓΡΑΦΙΑ -->
<h2 class="page-break">7. Βιβλιογραφία</h2>
<ol class="references">
  <li>
    F. M. Harper and J. A. Konstan, "The MovieLens Datasets: History and Context,"
    <em>ACM Transactions on Interactive Intelligent Systems (TiiS)</em>,
    vol. 5, no. 4, pp. 19:1–19:19, 2015.
    DOI: 10.1145/2827872
  </li>
  <li>
    Y. Koren, R. Bell, and C. Volinsky, "Matrix Factorization Techniques for Recommender Systems,"
    <em>IEEE Computer</em>, vol. 42, no. 8, pp. 30–37, 2009.
    DOI: 10.1109/MC.2009.263
  </li>
  <li>
    Y. Koren, "Factorization Meets the Neighborhood: A Multifaceted Collaborative Filtering Model,"
    in <em>Proceedings of the 14th ACM SIGKDD International Conference on Knowledge Discovery
    and Data Mining (KDD)</em>, Las Vegas, NV, USA, 2008, pp. 426–434.
  ...
  <li>
    ...
    in <em>The Adaptive Web: Methods and Strategies of Web Personalization</em>,
    P. Brusilovsky, A. Kobsa, and W. Nejdl, Eds. Berlin: Springer, 2007, pp. 325–341.
    DOI: 10.1007/978-3-540-72079-9_10
  </li>
  <li>
    P. Resnick, N. Iacovou, M. Suchak, P. Bergstrom, and J. Riedl,
    "GroupLens: An Open Architecture for Collaborative Filtering of Netnews,"
    in <em>Proceedings of the ACM Conference on Computer Supported Cooperative Work (CSCW)</em>,
    Chapel Hill, NC, USA, 1994, pp. 175–186.
  ...
  <li>
    N. Hug, "Surprise: A Python Library for Recommender Systems,"
    <em>Journal of Open Source Software</em>, vol. 5, no. 52, p. 2174, 2020.
    DOI: 10.21105/joss.02174
  </li>
  <li>
    X. He, L. Liao, H. Zhang, L. Nie, X. Hu, and T.-S. Chua,
    "Neural Collaborative Filtering,"
    in <em>Proceedings of the 26th International World Wide Web Conference (WWW)</em>,
    Perth, Australia, 2017, pp. 173–182.
